...
export default class VolumoProvider extends MetadataApiProvider {
	...
	readonly supportedUrls = new URLPattern({
		hostname: 'volumo.com',
		pathname:
			'/:region(\w{2})?/:type(album|artist|track|label)/{id}?:id(\d+)',
	});
	...
	override readonly features: FeatureQualityMap = {
		'cover size': 3000,
		'duration precision': DurationPrecision.US,
...
export class VolumoReleaseLookup extends ReleaseApiLookup<VolumoProvider, QobuzAlbum> {
	/**
